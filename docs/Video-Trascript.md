...
Our workflow has three main goals: produce accurate destination-level forecasts, prevent future-data leakage, and make the complete result reproducible from the submitted notebook. 

We compare simple baselines with recovery-aware statistical and foundation-model approaches, evaluate them using MASE and MAPE, select one final method, and export the final all-destination forecast directly from the notebook.
...
- `Date` is the first column. The other 20 columns are different destinations.
...
- Missing cells are never changed to zero because missing data does not mean zero demand. Leading pre-reporting gaps stay `NaN`; internal gaps are filled only in a separate modelling copy after the raw EDA identifies them.
...
- The first EDA separates leading gaps before reporting began from gaps inside an active country series. Leading gaps remain `NaN`; only internal gaps are imputed.
- The raw EDA finds exactly 24 internal missing months: 15 for Chile, 3 for Maldives, and 6 for Thailand. All other missing cells are before a destination's first reported month.
- Leaving those internal gaps blank would interrupt the monthly calendar and could cause lag features, rolling calculations, seasonal comparisons, and fitted models to lose rows, compress time, or fail.
- We create a separate modelling copy and linearly interpolate each internal gap between the nearest observed months. This is deterministic, transparent, and never replaces a missing value with zero.
- Imputation is repeated independently at each model cutoff. Therefore, no validation or assessment-period actual can be used to fill a training-period gap.
- The original raw tables and validation actuals remain unchanged. An imputation log records every changed destination, month, value, method, and bounding observed months.
- A second EDA verifies that internal gaps fall from 24 to zero while all 2,914 leading gaps remain missing. Overlay plots show the observed lines and all imputed points for the three affected destinations.
...
- All eight model generators receive the internally imputed, cutoff-safe modelling history. This prevents TimesFM's remaining `dropna()` preparation from compressing an internal calendar gap and lets Holt-Winters fit all 20 destinations without an internal-gap fallback.
...
[SCREEN: Show `raw_missingness_pattern_summary`, the 24-row imputation log, the before/after counts and overlay plot, then show the Q2 value-count fields and final zero-count audit fields.]
...
- The raw missingness EDA justifies imputing 24 internal gaps across Chile, Maldives, and Thailand. The post-imputation EDA confirms calendar continuity while preserving pre-reporting gaps.
...
- The ADF tests indicate that none of the 20 level series is stationary at the five-percent significance level. After first differencing, 18 of 20 are stationary; Chile and Czech remain non-stationary. This suggests that models must account for changing levels or differences rather than assuming a constant mean.
...
- **Result:** mean MASE `2.71`, median MASE `1.62`, and mean MAPE `45.7%`.
...
- **Result:** it ranked first on all three aggregate measures with mean MASE `2.48`, median MASE `1.53`, and mean MAPE `42.8%`.
...
- **Result:** mean MASE `2.96`, median MASE `2.65`, and mean MAPE `54.6%`.
...
- **Result:** mean MASE `4.07`, median MASE `2.63`, and mean MAPE `73.8%`.
...
- **Result:** mean MASE `2.61`, median MASE `2.00`, and mean MAPE `44.3%`. It ranked third on official mean MASE after imputation.
...
- **Implementation:** We will be using Ensemble technique here , we fit a damped additive trend and additive 12-month seasonality independently by destination, then combine a 30 percent Holt-Winters with 70 percent blended recovery. This is equivalent to 30 percent Holt-Winters, 42 percent lag-1, and 28 percent seasonal recovery.
- **Strength:** after imputation, Holt-Winters fits all 20 destinations without an internal-gap fallback and remains close to the selected model.
- **Limitation:** mean MASE `2.59`, median MASE `2.01`, and mean MAPE `43.8%` are all worse than blended recovery.
- **Result:** it ranks second on official mean MASE and remains a strong challenger, but is not selected because it does not lead any aggregate validation measure.
...
- Prophet and Holt-Winters record fallback diagnostics and fixed settings. After internal-gap imputation, both required zero fallbacks in official validation and the 12-month backtest.
...
| Seasonal recovery | 2.71 | 1.62 | 45.7% | Useful when annual seasonality remains important but the market is recovering to a new level. |
| Blended recovery | **2.48** | **1.53** | **42.8%** | Useful when both stability and recovery movement are needed. This was the selected model. |
| SARIMA | 2.96 | 2.65 | 54.6% | Useful when a series has enough history and reasonably stable autocorrelation and seasonal structure. |
| Prophet | 4.07 | 2.63 | 73.8% | Useful when recent trend changes and proportional seasonal movement are more important than fixed lag relationships. |
| TimesFM 2.5 | 2.61 | 2.00 | 44.3% | Useful as a quick zero-shot model when strong forecasts are needed without local fitting or manual order selection. |
| Holt-Winters recovery ensemble | 2.59 | 2.01 | 43.8% | Useful when smoothed level, trend, seasonality, and recovery anchoring are all desired. |
...
- **Holt-Winters ensemble:** imputation lets it fit all destinations and it ranks second on mean MASE, but it does not beat blended recovery on mean MASE, median MASE, or MAPE.

### Why blended recovery was selected

- **Best aggregate results:** it ranked first on mean MASE, median MASE, and mean MAPE after imputation.
...
- **Longer-horizon support:** its mean MASE was `2.18` in the 12-month recovery-era backtest, compared with `2.24` for lag-1.
...
- TimesFM ranked third in official validation with mean MASE `2.61` and mean MAPE `44.3%`.
- It achieved the best 12-month backtest mean MASE of `2.08`.
...
- We did not select it because blended recovery was better on mean MASE, median MASE, and mean MAPE in official validation and was easier to reproduce as the final submission model.

### Supporting backtest and caution

- The additional backtest uses a July 2022 cutoff and forecasts the next 12 months.
- TimesFM ranked first with mean MASE `2.08`.
- The Holt-Winters ensemble ranked second with mean MASE `2.17`, followed closely by blended recovery at `2.18`.
...
Historical missingness remains a limitation even after treatment. We preserve 2,914 leading pre-reporting gaps and linearly interpolate 24 internal gaps rather than treating them as zero. Linear interpolation restores calendar continuity but assumes smooth movement between bounding observations, which may be imperfect during the COVID disruption. Future work could compare state-space smoothing, seasonal interpolation, multiple imputation, or explicit missingness masks through rolling-origin validation.
...
- **Imputation evidence:** raw EDA found 24 internal gaps, and post-imputation EDA verified zero remaining internal gaps while preserving all leading gaps.
- **Final choice:** blended recovery achieved the best official mean MASE, median MASE, and mean MAPE after imputation.
...
In summary, we created a cutoff-safe and reproducible forecasting workflow for all 20 destinations. Raw EDA identified 24 internal gaps, which we imputed in a separate modelling copy before verifying calendar continuity with a second EDA. We used Q1 to generate the required baselines, Q2 to check table quality, and Q3 to compare forecast accuracy. Among eight candidates, blended recovery gave the best official mean MASE, median MASE, and mean MAPE while remaining stable and easy to reproduce. The Holt-Winters ensemble became fully usable across all destinations after imputation, and TimesFM remained strongest in the longer backtest. Our audited 12-month forecast passed all schema and value checks and can be exported directly from the notebook. The main next step would be wider rolling validation, alternative imputation sensitivity tests, and more destination-specific model selection. Thank you.
